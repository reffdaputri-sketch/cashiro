...
import 'package:cashiro/models/product_bundle_item.dart';
import 'package:cashiro/screens/scanner_screen.dart'; // Import Scanner
import 'package:cashiro/providers/auth_provider.dart';
import 'package:cashiro/screens/purchase_license_screen.dart';

import 'dart:io';
import 'dart:math';
import 'package:cashiro/services/database_service.dart' as db_service;
...
class _ProductFormScreenState extends State<ProductFormScreen> {
  ...
  void initState() {
    ...
    // Load products for bundle selection
    WidgetsBinding.instance.addPostFrameCallback((_) {
      final pProv = Provider.of<ProductProvider>(context, listen: false);
      setState(() {
         _allProducts = pProv.products.where((p) => !p.isBundle && p.id != widget.product?.id).toList();
      });
    });
  }
            ...
            content: Column(
              ...
              children: [
                DropdownButtonFormField<int>(
                  ...
                  items: _allProducts.map((p) => DropdownMenuItem(value: p.id!, child: Text(p.name))).toList(),
                  onChanged: (val) => setState(() => selectedProductId = val),
                ),
                const SizedBox(height: 16),
                TextField(
                  controller: qtyController,
                  decoration: const InputDecoration(labelText: 'Kuantitas', border: OutlineInputBorder()),
                  keyboardType: TextInputType.number,
                )
              ],
            ),
              ...
              ElevatedButton(
                onPressed: () {
                  ...
                  if (selectedProductId != null && qty > 0) {
                    final selectedProduct = _allProducts.firstWhere((p) => p.id == selectedProductId);
                    this.setState(() {
      ...
      child: ListView(
        children: [
          ...
          SwitchListTile(
            ...
            subtitle: const Text('Produk ini terdiri dari gabungan produk lain. Stok akan memotong komponen secara otomatis saat terjual.'),
            value: _isBundle,
          ...
          if (_isBundle) ...[
            ...
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                const Text('Komponen Paket', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                TextButton.icon(onPressed: _showBundleItemDialog, icon: const Icon(Icons.add), label: const Text('Tambah')),
              ],
            ),
            if (_bundleItems.isEmpty)
               const Text('Belum ada komponen yang ditambahkan. Silakan tambah.', style: TextStyle(color: Colors.grey))
            else
               ..._bundleItems.asMap().entries.map((entry) {
                 final idx = entry.key;
                 final b = entry.value;
                 return ListTile(
                   title: Text(b.itemName ?? 'Produk ID ${b.itemProductId}'),
                   subtitle: Text('Kuantitas: ${b.quantity}'),
                   trailing: IconButton(
                     icon: const Icon(Icons.delete, color: Colors.red),
                     onPressed: () {
                       setState(() => _bundleItems.removeAt(idx));
                     },
                   ),
                 );
               }),

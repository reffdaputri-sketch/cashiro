...
import 'package:cashiro/screens/staff_list_screen.dart';
import 'package:cashiro/screens/report_screen.dart';
import 'package:cashiro/screens/history_screen.dart';
import 'package:cashiro/screens/master_data_screen.dart';
import 'package:cashiro/screens/stock_report_screen.dart';
import 'package:cashiro/screens/online_store_screen.dart';
import 'package:cashiro/screens/cash_flow_screen.dart';
import 'package:cashiro/screens/expense_screen.dart';
import 'package:cashiro/screens/referral_management_screen.dart';
import 'package:cashiro/screens/edit_store_screen.dart';
import 'package:cashiro/services/database_service.dart';
import 'package:cashiro/services/report_service.dart';
...
class _DashboardScreenState extends State<DashboardScreen> {
                            ...
                            child: Column(
                              ...
                              children: [
                                FittedBox(
                                  ...
                                  child: Text('$greeting, $userName! 👋', style: const TextStyle(color: Colors.white, fontSize: 22, fontWeight: FontWeight.bold)),
                                ),
                ...
                Positioned(
                  ...
                  child: SizedBox(
                    ...
                    child: ListView(
                      ...
                      children: [
                        _buildMetricCard(context, 'Penjualan', NumberFormat.currency(locale: 'id_ID', symbol: 'Rp ', decimalDigits: 0).format(_todaySales), _metricPeriod, Icons.shopping_bag, Colors.green, true),
                        const SizedBox(width: 12),
                        _buildMetricCard(context, 'Total Transaksi', '$_todayTxCount', _metricPeriod, Icons.shopping_cart_outlined, Colors.blue, true),
                        const SizedBox(width: 12),
                        _buildMetricCard(context, 'Laba Bersih', NumberFormat.currency(locale: 'id_ID', symbol: 'Rp ', decimalDigits: 0).format(_todayProfit), _metricPeriod, Icons.trending_up, Colors.green, true),
                        const SizedBox(width: 12),
                        _buildMetricCard(context, 'Total Stok Item', '$_totalStock', 'Seluruh Produk', Icons.inventory_2_outlined, Colors.orange, false),
                      ],
                    ),
                  ),
                ),
  ...
  Widget _buildMetricCard(BuildContext context, String title, String value, String subtitle, IconData icon, Color iconColor, bool isUp) {
    ...
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text(title, style: TextStyle(fontSize: isDesktop ? 14 : 12, color: Colors.black87, fontWeight: FontWeight.w600)),
          const Spacer(),
          FittedBox(
            fit: BoxFit.scaleDown,
            alignment: Alignment.centerLeft,
            child: Text(value, style: TextStyle(fontSize: isDesktop ? 24 : 18, fontWeight: FontWeight.bold)),
          ),
          const Spacer(),
          Row(
            children: [
              Icon(isUp ? Icons.arrow_upward : Icons.info_outline, size: isDesktop ? 16 : 12, color: isUp ? Colors.green : Colors.grey),
              const SizedBox(width: 6),
              Expanded(child: Text(subtitle, style: TextStyle(fontSize: isDesktop ? 12 : 10, color: isUp ? Colors.green : Colors.grey), overflow: TextOverflow.ellipsis)),
            ],
          )
        ],
      ),
  ...
  Widget _buildMenuBtn(BuildContext context, String title, IconData icon, VoidCallback onTap) {
    // Determine screen width to adjust size for desktop vs mobile
    final screenWidth = MediaQuery.of(context).size.width;
    final isDesktop = screenWidth > 800;
  ...
  Widget _buildTransactionItem(BuildContext context, String id, String amount, String date) {
    final isDesktop = MediaQuery.of(context).size.width > 800;
    return Padding(
        ...
        children: [
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(id, style: TextStyle(fontWeight: FontWeight.bold, fontSize: isDesktop ? 14 : 11)),
                const SizedBox(height: 4),
                Text(date, style: TextStyle(color: Colors.grey, fontSize: isDesktop ? 12 : 9)),
              ],
            ),
          ),
          Column(
            ...
            children: [
              Text(amount, style: TextStyle(fontWeight: FontWeight.bold, fontSize: isDesktop ? 14 : 11)),
              const SizedBox(height: 4),
              Container(
  ...
  Widget _buildStockItem(BuildContext context, String name, String stock, Color bgColor, IconData icon) {
    final isDesktop = MediaQuery.of(context).size.width > 800;
    return Padding(
      padding: const EdgeInsets.only(bottom: 12),
      child: Row(
        children: [
          Container(
            ...
            child: Icon(icon, size: isDesktop ? 24 : 16, color: Colors.black54),
          ),
          const SizedBox(width: 12),
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(name, style: TextStyle(fontWeight: FontWeight.bold, fontSize: isDesktop ? 14 : 11), maxLines: 1, overflow: TextOverflow.ellipsis),
                const SizedBox(height: 4),
                Text('Stok: $stock', style: TextStyle(color: Colors.grey, fontSize: isDesktop ? 12 : 9)),
              ],
            ),
          ),
          Container(
            width: isDesktop ? 28 : 20, height: isDesktop ? 28 : 20,
            alignment: Alignment.center,
            decoration: BoxDecoration(color: Colors.red.withOpacity(0.1), shape: BoxShape.circle),
            child: Text(stock, style: TextStyle(color: Colors.red, fontSize: isDesktop ? 12 : 10, fontWeight: FontWeight.bold)),
          )
        ],
      ),
    );
        ...
        children: [
          ...
          if (_isLoadingData)
            ...
            ..._recentTransactions.map((tx) {
              ...
              return _buildTransactionItem(context, tx['receipt_number'] ?? 'INV', amount, formattedDate);
            }).toList(),
          ...
          InkWell(
            ...
            child: Container(
              ...
              child: Row(
                mainAxisAlignment: MainAxisAlignment.center,
                children: const [
                  Text('Lihat semua transaksi', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600)),
                  SizedBox(width: 4),
        ...
        children: [
          ...
          if (_isLoadingData)
            ...
            ..._lowStockProducts.map((p) {
              return _buildStockItem(context, p['name'] ?? 'Produk', p['stock'].toString(), Colors.orange.shade100, Icons.inventory_2);
            }).toList(),
        ],
      ),
    );

...
class ProductProvider with ChangeNotifier {
  ...
  Future<void> fetchProducts() async {
    ...
    final bundleItemMaps = await _db.getAll('product_bundles');
    ...
    _products = productMaps.map((pMap) {
       ...
       final variations = variationMaps
           ...
           .toList();
       final bundleItems = bundleItemMaps
           .where((b) => b['bundle_product_id'] == pId)
           .map((b) => ProductBundleItem.fromMap(b))
           .toList();
       return Product.fromMap(pMap, variations: variations, bundleItems: bundleItems);
    }).toList();
  ...
  Future<void> updateStock(int id, int newStock, {int? supplierId, String? notes}) async {
    final product = await _db.getById('products', id);
    if (product != null) {
      ...
      if (diff != 0) {
        await _db.insert('stock_opname_history', {
          'product_id': id,

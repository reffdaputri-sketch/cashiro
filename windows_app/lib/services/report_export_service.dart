...
class ReportExportService {
  ...
  static Future<void> exportToExcel(DateTime start, DateTime end, String rangeLabel) async {
    ...
    // Title block
    sheet.appendRow([TextCellValue('LAPORAN KEUANGAN CASHIRO')]);
    ...
    if (fileBytes != null) {
      ...
      final tempFile = File('${tempDir.path}/laporan_cashiro_${sanitizedLabel}.xlsx');
      await tempFile.writeAsBytes(fileBytes);
      
      await Share.shareXFiles(
        [XFile(tempFile.path)],
        subject: 'Laporan Keuangan Cashiro ($rangeLabel)',
      );
    }
  }
  ...
  static Future<void> exportToPdf(DateTime start, DateTime end, String rangeLabel) async {
            ...
            child: pw.Row(
              ...
              children: [
                pw.Column(
                  ...
                  children: [
                    pw.Text('CASHIRO POS', style: pw.TextStyle(font: fontBold, fontSize: 24, color: PdfColors.teal)),
    ...
    final tempDir = await getTemporaryDirectory();
    final sanitizedLabel = rangeLabel.replaceAll(' ', '_').toLowerCase();
    final tempFile = File('${tempDir.path}/laporan_cashiro_${sanitizedLabel}.pdf');
